...
async function postContent(content,title,dbname){
    //console.log(title+"---"+content)
            confluence.getContentByPageTitle("NTC", title,function(err, data) {
                if(data.results!=undefined&&data.results[0]){
                    ...
                    confluence.putContent("NTC",id,version,title,content,function(err,data){
                    console.log(err)
                    
            })}
            else{
                confluence.postContent("NTC",title,content,null,function(err,data){
                    console.log(err)
